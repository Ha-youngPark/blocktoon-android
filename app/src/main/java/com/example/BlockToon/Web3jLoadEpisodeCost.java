...
/*에피소드별 가격 조회*/
public class Web3jLoadEpisodeCost extends AsyncTask<String, Void, List<String>> {
    List<String> result;
    ...
    protected List<String> doInBackground(String... args) {
        ...
        try {
            result = contract.Episode_getMoney(args[1]).send();
            Log.d("test",result.get(0));
            Log.d("test",result.get(1).toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
